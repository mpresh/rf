...
CONSUMER_KEY = getattr(settings, 'lDaSawD84v8gQwuRYvCg', 'YOUR_KEY')
CONSUMER_SECRET = getattr(settings, 'V5EN5wEUujALOwPPzF7LDW9MfLbKsg3i2qnucz3Z0c', 'YOUR_SECRET')
...
def get_unauthorised_request_token(consumer, connection, signature_method=signature_method):
    print "consumer", consumer, connection, signature_method
    oauth_request = oauth.OAuthRequest.from_consumer_and_token(
        consumer, http_url=REQUEST_TOKEN_URL
    )
    print "oauth_request", oauth_request
    oauth_request.sign_request(signature_method, consumer, None)
    print "aaa", oauth_request, connection
